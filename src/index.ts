...
function attachHandlers(nsp: ReturnType<Server['of']>) {
  nsp.on('connection', (socket) => {
    ...
    socket.on('startGame', (payload: any, ack?: (r: any) => void) => {
      ...
      if (!res.success) {
        ack?.({ success: false, error: res.error || 'Only host can start / not enough players / already started' });
        return;
      }
      ack?.({ success: true });
    ...
    socket.on('playerAction', (payload: any, ack?: (r: any) => void) => {
      ...
      if (!res.success) {
        ack?.({ success: false, error: res.error || 'Invalid action / not your turn / lobby not found' });
        return;
      }
