...
export class UnoGame {
  ...
  createLobby(hostIdArg: string, nickname: string, avatarUrl: string | null): string {
  ...
  handleAction(code: string, odotpid: string, action: UnoPlayerAction): { success: boolean; error?: string } {
    const lobby = this.lobbies.get(code);
    if (!lobby) return { success: false, error: 'Lobby not found' };
    ...
    if (action.type === 'draw') {
      ...
      this.lobbies.set(code, this.bumpState(res.state));
      this.onStateUpdate(code);
      return { success: true };
    }
  ...
  private applyCallUno(
    ...
    let next: UnoGameState = { ...lobby, mustCallUno: null };
